...
Map.prototype.buttonClickedFromMarker = function(marker) {
	for (var i = 0, length = this.arrayOfFakeLatLng.length; i < length; i++) {
		if (marker.getPosition().lat().toFixed(2) === this.arrayOfFakeLatLng[i]["place"]["lat"].toFixed(2)
			&& marker.getPosition().lng().toFixed(2) === this.arrayOfFakeLatLng[i]["place"]["lng"].toFixed(2)) {
